...
namespace RescueDog
{
    public class Editor
    {
        ...
        public SaveLoadLevel saveLoadLevel; //class used to save and load level data
        ...
        public Editor(GameScreen GameScreen)
        {
            ...
            saveLoadLevel = new SaveLoadLevel(gameScreen);
            ...
            buttonSave = new Button(gameScreen.screenManager, "save",
                new Point(buttonDrawDebug.rectangle.X + buttonDrawDebug.rectangle.Width + buttonPadding, 6), new Point(60, buttonHeight));
            buttonLoad = new Button(gameScreen.screenManager, "load",
                new Point(buttonSave.rectangle.X + buttonSave.rectangle.Width + buttonPadding, 6), new Point(60, buttonHeight));
            buttonNewLevel = new Button(gameScreen.screenManager, "new",
                new Point(buttonLoad.rectangle.X + buttonLoad.rectangle.Width + buttonPadding, 6), new Point(60, buttonHeight));
            ...
            buttons.Add(buttonSave);        //this is done in handle input() below
            buttons.Add(buttonLoad);
            buttons.Add(buttonNewLevel);
                ...
                {   //loop through all buttons on the buttons list
                    ...
                    {   //set the current buttons over state if the cursor collides with it's background rec
                        ...
                        {   //if user clicked on the button, set the buttons rec's draw color to downCOlor
                            ...
                            else if (buttons[counter] == buttonSave) { saveLoadLevel.SaveLevel(); }
                            else if (buttons[counter] == buttonLoad) { saveLoadLevel.LoadLevel(); }
